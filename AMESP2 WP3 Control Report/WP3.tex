    ...
    \Procedure{GetOscilloscopeMeasurement}{\null}
        ...
        \ForAll {f \textbf{in} fft\_result}
            \If{$f \ge$ low\_pass\_cutoff}
                \State $f \gets 0$ 
            \EndIf
            \If{$f >$ bandstop\_start \textbf{or} $f <$ bandstop\_endt}
                \State $f \gets 0$ 
            \EndIf
        \EndFor
        \Statex
...
and \codeword{ClassifyEHDA}. The former is responsible for acquiring and filtering the signal of $i_{GND}$ read,
